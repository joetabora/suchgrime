    ...
    <section id="contact" className="border-t-4 border-parlor-accent bg-bg">
      <div className="mx-auto grid max-w-[1400px] border-x border-white/10 lg:grid-cols-2">
        <div className="border-b border-white/10 p-6 md:p-12 lg:border-b-0 lg:border-r">
          ...
          >
            <p className="text-label mb-4">{parlor.contact.note}</p>
            <h2 className="font-display text-5xl leading-[0.9] tracking-wide md:text-7xl lg:text-8xl">
              START
              <br />A PROJECT
            </h2>
            <p className="mt-6 max-w-md text-muted">
              Tell us about your brand, your goals, and the vibe you&apos;re going for.
            </p>
            <a
              href={`mailto:${parlor.contact.email}`}
        ...
        <div className="p-6 md:p-12">
          ...
          >
            <form onSubmit={handleSubmit} noValidate className="space-y-6">
              <div>
                <label htmlFor="parlor-name" className="mb-1.5 block font-mono text-xs uppercase tracking-widest text-muted">
                  Name
                </label>
                <input
                  id="parlor-name"
                ...
                />
                {errors.name && (
                  <p className="mt-1 text-sm text-parlor-accent">{errors.name}</p>
                )}
              </div>

              <div>
                <label htmlFor="parlor-email" className="mb-1.5 block font-mono text-xs uppercase tracking-widest text-muted">
                  Email
                </label>
                <input
                  id="parlor-email"
                ...
                />
                {errors.email && (
                  <p className="mt-1 text-sm text-parlor-accent">{errors.email}</p>
                )}
              </div>

              <div>
                <label htmlFor="parlor-type" className="mb-1.5 block font-mono text-xs uppercase tracking-widest text-muted">
                  Project Type
                </label>
                <select
                  id="parlor-type"
                ...
                >
                  ...
                  {parlor.contact.projectTypes.map((type) => (
              ...
              <div>
                <label htmlFor="parlor-message" className="mb-1.5 block font-mono text-xs uppercase tracking-widest text-muted">
                  Message
                </label>
                <textarea
                  id="parlor-message"
              ...
              <button
                type="submit"
                className="w-full border-2 border-parlor-accent bg-parlor-accent py-4 font-display text-2xl tracking-wider text-text transition-colors hover:bg-transparent hover:text-parlor-accent"
              >
                SEND INQUIRY
              </button>
            </form>
