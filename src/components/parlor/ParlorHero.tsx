...
const headlineLines = ["URBAN", "WEB", "CRAFT"]

export function ParlorHero() {
  const pills = [...parlor.marqueePhrases, ...parlor.marqueePhrases]
        ...
        >
          ...
          <h1 className="font-display leading-[0.85] tracking-wide">
            {headlineLines.map((line) => (
              <span
                key={line}
                className="block text-[clamp(4rem,15vw,11rem)]"
              >
                {line}
              </span>
            ))}
      ...
      >
        <div className="mx-auto grid max-w-[1400px] border-white/10 lg:grid-cols-2 lg:border-x">
          <div className="border-b border-white/10 p-6 lg:border-b-0 lg:border-r lg:p-8">
            <p className="text-label mb-2">{parlor.tagline}</p>
            <p className="max-w-md text-muted">{parlor.heroSubtext}</p>
