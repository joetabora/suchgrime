...
export function ParlorWork() {
  const liveProjects = parlor.work.filter((p) => p.live)
  const comingSoon = parlor.work.filter((p) => !p.live)
  ...
  return (
    <section id="work" className="poster-section bg-bg">
      <div className="border-b border-white/10 px-6 py-12 md:px-12">
        ...
        <h2 className="font-display text-6xl tracking-wide md:text-8xl">FROM THE PARLOR</h2>
      </div>
      ...
      {liveProjects.map((project, i) => (
        ...
        >
          <div className="mx-auto max-w-[1400px] border-x border-white/10">
            ...
            <div className="aspect-[21/9] overflow-hidden border-b border-white/10">
              {"kind" in project && project.kind === "app" ? (
